...
description: "正则表达式是程序员最不想手写、又不得不手写的东西之一。语法诡异、调试痛苦、改一行炸一片。但在 Agent 系统里，让 LLM 生成正则这件事，不只是\"帮我写个 pattern\"这么简单——它改变了 Agent 和非结构化数据打交道的方式。"
keywords: "agent, 为什么需要自己写正则？, llm, regex, pattern, AI工程"
---
