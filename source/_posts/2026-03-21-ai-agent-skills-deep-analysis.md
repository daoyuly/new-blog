...
### Q: 什么是 AI Agent Skills？

A: AI Agent Skills 是将工具使用能力抽象为可复用、可组合的知识单元。它以结构化形式（如 Markdown 文件）存储工具调用、示例和约束，解决传统工具调用的上下文丢失和知识碎片化问题，实现代理能力的持久化与演化。

### Q: AI Agent Skills 和传统工具调用有什么区别？

A: 传统工具调用是独立且无状态的，缺乏历史经验保留，容易引入回归错误。而 Skills 采用“有状态提示”机制，通过读写反思学习将经验固化为技能。它无需更新 LLM 参数即可实现持续学习，性能最高可提升 116.2%。

### Q: Memento-Skills 框架是如何实现“代理设计代理”的？

A: Memento-Skills 通过读写反思学习机制实现。在 Read 阶段，技能路由器选择最相关技能；在 Write 阶段，基于新经验提取并更新技能库。这种机制让代理能自主生成和优化外部技能，实现无需参数更新的持续演化。

### Q: 如何在实际开发中构建一个标准的 AI Agent Skill？

A: 开发者需定义技能名称、描述、依赖工具、使用示例和约束条件。例如构建飞书文档操作技能时，需声明 feishu_doc 工具，提供“读取文档”等示例，并明确 API 权限和支持操作类型，确保能力边界清晰。

### Q: AI Agent 适合使用 Skills 框架吗？如何选择？

A: 适合需要长期记忆和复杂任务拆解的场景。若任务频繁依赖历史经验且涉及多工具组合，建议采用 Skills 框架（如 Memento-Skills 或 SKILLS）以提升准确率；若是简单单次查询，传统工具调用即可满足需求。
---
